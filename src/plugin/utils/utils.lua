local io = require("plugin.utils.io")
local json = require("plugin.utils.json")
_M = {}
...
function _M.load_conf(path)
    if path == nil or path == "" then
        return nil, "require conf path!"
    end
    local conf_str = io.read_file(path)
    return json.decode(conf_str)
end
